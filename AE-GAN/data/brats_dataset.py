...
class BratsDataset(BaseDataset):
    def __init__(self, opt):
        ...
        ct_path = os.path.join(data_root, 'ct')
        xray_path = os.path.join(data_root, 'xray')
        # t2_path = os.path.join(data_root, 't2')

        # self.flair_set = load_set(flair_path)
        self.ct_set = load_set(ct_path)
        self.xray_set = load_set(xray_path)
        # self.t2_set = load_set(t2_path)

        self.n_data = len(self.ct_set)#有多少个数据

        # 2. create modal mask
        # modal_names = ['t1', 't1ce', 't2', 'flair']#几个模态
        modal_names = ['ct', 'xray']
        n_modal = len(modal_names)#多少个模态
