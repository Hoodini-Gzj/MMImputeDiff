...
class MixedFusion_Block(nn.Module):
    ...
    def __init__(self,in_dim, out_dim,act_fn):
        ...
        self.layer1 = nn.Sequential(nn.Conv3d(in_dim*3, in_dim, kernel_size=3, stride=1, padding=1),nn.BatchNorm3d(in_dim),act_fn,)
        
        # revised in 09/09/2019.
        #self.layer1 = nn.Sequential(nn.Conv3d(in_dim*3, in_dim,  kernel_size=1),nn.BatchNorm3d(in_dim),act_fn,)
        self.layer2 = nn.Sequential(nn.Conv3d(in_dim*2, out_dim, kernel_size=3, stride=1, padding=1),nn.BatchNorm3d(out_dim),act_fn,)
...
class MixedFusion_Block0(nn.Module):
    def __init__(self,in_dim, out_dim,act_fn):
        ...
        self.layer1 = nn.Sequential(nn.Conv3d(in_dim*3, in_dim, kernel_size=3, stride=1, padding=1),nn.BatchNorm3d(in_dim),act_fn,)
        #self.layer1 = nn.Sequential(nn.Conv3d(in_dim*3, in_dim, kernel_size=1),nn.BatchNorm3d(in_dim),act_fn,)
        self.layer2 = nn.Sequential(nn.Conv3d(in_dim, out_dim, kernel_size=3, stride=1, padding=1),nn.BatchNorm3d(out_dim),act_fn,)
...
class Multi_modal_generator(nn.Module):
    ...
    def __init__(self,input_nc, output_nc, ngf):
        ...
        self.down_1_0 = nn.Sequential(
                nn.Conv3d(in_channels=self.in_dim,  out_channels=self.out_dim, kernel_size=3, stride=1, padding=1),nn.BatchNorm3d(self.out_dim), act_fn,
                nn.Conv3d(in_channels=self.out_dim, out_channels=self.out_dim, kernel_size=3, stride=1, padding=1),nn.BatchNorm3d(self.out_dim), act_fn,
                )
        self.pool_1_0 = maxpool()
        
        self.down_2_0 = nn.Sequential(
                nn.Conv3d(in_channels=self.out_dim,  out_channels=self.out_dim*2, kernel_size=3, stride=1, padding=1),nn.BatchNorm3d(self.out_dim*2), act_fn,
                nn.Conv3d(in_channels=self.out_dim*2,out_channels=self.out_dim*2, kernel_size=3, stride=1, padding=1),nn.BatchNorm3d(self.out_dim*2), act_fn,
                )
        self.pool_2_0 = maxpool()
        
        self.down_3_0 = nn.Sequential(
                nn.Conv3d(in_channels=self.out_dim*2,  out_channels=self.out_dim*4, kernel_size=3, stride=1, padding=1),nn.BatchNorm3d(self.out_dim*4), act_fn,
                nn.Conv3d(in_channels=self.out_dim*4,out_channels=self.out_dim*4, kernel_size=3, stride=1, padding=1),nn.BatchNorm3d(self.out_dim*4), act_fn,
                )
        ...
        # down 4th layer
        self.down_fu_4 = nn.Sequential(nn.Conv3d(in_channels=self.out_dim*4,  out_channels=self.out_dim*8, kernel_size=3, stride=1, padding=1),nn.BatchNorm3d(self.out_dim*8), act_fn,)     
        ...
        self.out        = nn.Sequential(nn.Conv3d(int(self.out_dim),1, kernel_size=3, stride=1, padding=1),nn.Tanh()) #  self.final_out_dim
        ...
        self.out1       = nn.Sequential(nn.Conv3d(int(self.out_dim),1, kernel_size=3, stride=1, padding=1),nn.Tanh()) #  self.final_out_dim
        ...
        self.out2       = nn.Sequential(nn.Conv3d(int(self.out_dim),1, kernel_size=3, stride=1, padding=1),nn.Tanh()) #  self.final_out_dim
